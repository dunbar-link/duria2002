...
## 7-7. P4-1E 확정 — voice signal Point 정책 (2026-07-05)

**결정**: 2초 음성 신호는 emoji 신호와 동일하게 기존 `signalDays`("하루 신호 보낸 날" +5P) 안에 포함되는 것으로 충분하다. voice 전용 별도 보너스는 만들지 않는다. 이유: 포인트 인플레이션 방지, 보상 때문에 음성을 남발하는 유인 차단, "관계 유지 행동"이라는 본질 유지, MVP에서는 보상보다 2초 신호 UX 검증이 우선.

**코드 조사 결과 — 이미 정책대로 구현돼 있어 코드 변경 없음**:
- `app/api/me/signal-days/route.ts`: `signals` 테이블을 `sender_id IN (내 계정 전체 legacy id)` 로만 필터하고 `type` 컬럼은 전혀 건드리지 않는다 → emoji/voice 구분 없이 둘 다 같은 집계에 들어간다. 별도 voice bonus 쿼리/컬럼 자체가 없다.
- 같은 KST 날짜의 신호는 `toKstDateKey(created_at)` 값을 `Set<string>`에 넣어 dedupe → 하루에 emoji만 보내도, voice만 보내도, 둘 다 보내도 그 날은 `signalDayCount` 1만 늘어난다(A/B/C 케이스 모두 이미 1일로 정확히 계산됨, D도 다른 날이면 정상적으로 +1).
- `sender_id` 필터라 내가 **받은** 신호(상대가 보낸 voice/emoji)는 애초에 이 집계에 안 들어간다(E 케이스 이미 충족).
- `app/dashboard/_components/me/point-ledger.ts`(`buildDeterministicPointScore`): `signalDayCount * POINT_RULES.signalDay(5)`가 signal 관련 point의 전부. voice 전용 필드/가산 코드 없음.
- 🪙 point burst 효과(`me/page.tsx`)는 "이 기기에서 마지막으로 본 total" 대비 total이 실제로 늘었을 때만 뜬다(localStorage seen-cache 비교). `signalDayCount`가 하루에 한 번만 늘어나므로 같은 날 emoji+voice를 연달아 보내도 burst가 두 번 뜨지 않는다(F 케이스 이미 충족).
- UI 문구 검사: 2초 신호방(`two-second-room.tsx`)·상세페이지 어디에도 "음성 전용 +5P/보너스" 같은 문구 없음. Me 화면의 `+5P` 힌트는 프로필 추가정보 필드용이고 signal과 무관 — 오해 소지 없음.

**video 보상**: 아직 구현 전이므로 보류 유지. 영상이 들어와도 별도 보상은 신중히(P4-2 설계 이후 별도 판단).

**변경**: 코드 변경 없음(정책이 이미 구현과 일치). 이 섹션이 정책 확정 기록.

**검증**: `npx tsc --noEmit` PASS, `npm run build` PASS, `verify-home-regression.mjs` 14/14 PASS(변경 없음 확인 목적).
