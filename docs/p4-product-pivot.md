...
**미해결 → 분리**
- [P4-sync hotfix] Home/People 연결 hydration 을 계정 전체 id 로: 진단상 store 의 inviterPeople 경로가 A person 을 만들어야 정상이나 실사용에서 누락 → 계정 B 로 로그인한 런타임 store 추적 필요. 회귀 잠금된 store/Home 영역이라 별도 신중 작업.
