...
const OUT = path_1.default.join(SEED_DIR, "edges_celebrity_seed.csv");
...
function upsertMinimalNodesIntoCore() {
    const core = path_1.default.join(SEED_DIR, "orgs_kr_core.csv");
    if (!fs_1.default.existsSync(core)) {
        // core가 없으면 하나 만든다 (프로젝트가 이미 있으면 보통 존재함)
        fs_1.default.writeFileSync(core, "pid,name,category,country,city,industry\n", "utf-8");
    }
    const text = fs_1.default.readFileSync(core, "utf-8");
    ...
    if (toAppend.length > 0) {
        ...
        fs_1.default.writeFileSync(core, text + appendText, "utf-8");
...
function writeCelebrityEdges(orgIndex) {
    ...
    const edges = [];
    const dedup = new Set();
    function addEdgeBySrcName(srcName, dstPid, edgeType, weight, meta) {
    ...
    fs_1.default.writeFileSync(OUT, lines.join("\n") + "\n", "utf-8");
    console.log("✅ created:", path_1.default.relative(ROOT, OUT));
