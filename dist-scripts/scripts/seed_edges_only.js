...
dotenv_1.default.config({
    path: path_1.default.join(process.cwd(), ".env.local"),
    override: true,
});
const ROOT = process.cwd();
const SEED_DIR = path_1.default.join(ROOT, "seed");
const SUPABASE_URL = (process.env.SUPABASE_URL ?? "").trim();
const SUPABASE_SERVICE_ROLE_KEY = (process.env.SUPABASE_SERVICE_ROLE_KEY ?? "").trim();
if (!SUPABASE_URL)
    throw new Error("Missing env: SUPABASE_URL (check .env.local)");
if (!/^https?:\/\//i.test(SUPABASE_URL))
    throw new Error(`Invalid SUPABASE_URL: ${JSON.stringify(SUPABASE_URL)}`);
if (!SUPABASE_SERVICE_ROLE_KEY)
    throw new Error("Missing env: SUPABASE_SERVICE_ROLE_KEY (check .env.local)");
...
function pickIndex(header, candidates) {
    ...
    for (let i = 0; i < h.length; i++) {
        for (const c of candidates) {
            if (h[i].includes(norm(c)))
                return i;
        }
    }
    return -1;
}
function findEdgeFiles() {
    if (!fs_1.default.existsSync(SEED_DIR))
        throw new Error(`Missing seed dir: ${SEED_DIR}`);
    return fs_1.default
        .readdirSync(SEED_DIR)
        .filter((f) => f.startsWith("edges_") && f.endsWith(".csv"))
        .map((f) => path_1.default.join(SEED_DIR, f));
}
function pidToType(pid) {
    const p = pid.trim();
    if (p.startsWith("org:"))
        return "org";
    if (p.startsWith("u_") || p.startsWith("user:") || p.startsWith("me"))
        return "person";
    if (p.startsWith("celeb:"))
        return "celebrity";
    if (/^KR-(UNIV|COMP|CITY|IND)/i.test(p))
        return "org";
    return "entity";
}
function toWeight(v, fallback) {
    const t = (v ?? "").trim();
    if (!t)
        return fallback;
    const n = Number(t);
...
async function main() {
    ...
    console.log("✅ edge files:", files.map((f) => path_1.default.basename(f)));
    ...
    for (const file of files) {
        ...
        if (header.length === 0) {
            skippedFiles.push({ file: path_1.default.basename(file), reason: "empty header" });
            continue;
        }
        ...
        if (srcI < 0 || dstI < 0 || typeI < 0) {
            skippedFiles.push({ file: path_1.default.basename(file), reason: "missing required columns", header });
            continue;
        }
    ...
    if (skippedFiles.length > 0)
        console.log("⚠️ skippedFiles:", JSON.stringify(skippedFiles, null, 2));
    if (allRows.length === 0) {
