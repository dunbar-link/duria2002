...
const CSV_FILES = [
  path.join(PROJECT_ROOT, "seed", "edges_celebrity_autogen.csv"),
  path.join(PROJECT_ROOT, "seed", "edges_bridge_manual.csv"),
];
...
function normalizeOrgPid(raw: string): string {
  const v = raw.trim();
  if (!v) return "";

  const lower = v.toLowerCase();

  if (lower === "org:kr-univ-korea") return KOREA_UNIV_PID;
  if (lower === "org:univ:korea-university") return KOREA_UNIV_PID;
  if (lower.includes("korea-university")) return KOREA_UNIV_PID;
  if (lower.includes("korea university")) return KOREA_UNIV_PID;
  if (lower.includes("고려대학교")) return KOREA_UNIV_PID;
...
function normalizeCelebPid(raw: string): string {
  const v = raw.trim();
  if (!v) return "";
...
function normalizeAnyPid(raw: string): string {
  const v = raw.trim();
  if (!v) return "";

  if (v.startsWith("celeb:")) return normalizeCelebPid(v);
  if (v.startsWith("org:")) return normalizeOrgPid(v);
...
function toEdgeRow(row: CsvRow): DlEdgeRow {
  ...
  const id =
    (row.id ?? "").trim() || stableUuidFromPair(from_pid, to_pid, label);
...
async function upsertLike(rows: DlEdgeRow[]) {
  let inserted = 0;
  let updated = 0;
...
function loadAllRows(): DlEdgeRow[] {
  ...
  const dedupe = new Map<string, DlEdgeRow>();
