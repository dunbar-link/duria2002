...
alter table public.dl_graph_expansion_candidates
  add column if not exists bridge_pid text;

alter table public.dl_graph_expansion_candidates
  add column if not exists last_execution_log jsonb;
...
update public.dl_graph_expansion_candidates
set bridge_pid = concat('bridge:', owner_user_id::text, ':', bridge_candidate_id_key)
where bridge_pid is null
  and bridge_candidate_id_key is not null
  and owner_user_id is not null;

update public.dl_graph_expansion_candidates
set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
  'bridgePid',
  concat('bridge:', owner_user_id::text, ':', bridge_candidate_id_key)
)
where (metadata ->> 'bridgePid') is null
  and bridge_candidate_id_key is not null
  and owner_user_id is not null;

create table if not exists public.dl_wallet_balances (
...
create table if not exists public.dl_wallet_ledger (
...
create index if not exists idx_dl_wallet_ledger_user_id_created_at
  on public.dl_wallet_ledger (user_id, created_at desc);
...
begin
  ...
  v_bridge_pid := coalesce(
    v_candidate.bridge_pid,
    v_candidate.metadata ->> 'bridgePid',
    case
      when v_candidate.bridge_candidate_id_key is not null and v_candidate.owner_user_id is not null
        then concat('bridge:', v_candidate.owner_user_id::text, ':', v_candidate.bridge_candidate_id_key)
      else null
    end
  );

  v_target_pid := coalesce(v_candidate.target_pid, v_candidate.metadata ->> 'targetPid');
  v_target_name := coalesce(v_candidate.target_name, v_candidate.metadata ->> 'targetName', 'Unknown');
  v_edge_label := coalesce(v_candidate.edge_label, v_candidate.metadata ->> 'edgeLabel', 'graph_expansion_bridge');
  v_trust := coalesce(v_candidate.trust, nullif(v_candidate.metadata ->> 'trust', '')::integer, 70);
  v_tier := coalesce(v_candidate.tier, nullif(v_candidate.metadata ->> 'tier', '')::integer, 50);
  ...
  update public.dl_graph_expansion_candidates
  set
    bridge_pid = coalesce(bridge_pid, v_bridge_pid),
    metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('bridgePid', v_bridge_pid)
  where id = p_candidate_id;
