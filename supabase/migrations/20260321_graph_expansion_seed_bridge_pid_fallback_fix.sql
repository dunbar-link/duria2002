...
set bridge_pid = null
where bridge_pid is not null
  and right(bridge_pid, 1) = ':';

update public.dl_graph_expansion_candidates
set bridge_pid = metadata ->> 'bridge_pid'
where bridge_pid is null
  and coalesce(metadata ->> 'bridge_pid', '') <> '';

update public.dl_graph_expansion_candidates
set metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
  'bridgePid',
  coalesce(
    nullif(metadata ->> 'bridgePid', ''),
    nullif(metadata ->> 'bridge_pid', ''),
    bridge_pid
  )
)
where coalesce(
...
begin
  ...
  v_bridge_pid := coalesce(
    nullif(v_candidate.bridge_pid, ''),
    nullif(v_candidate.metadata ->> 'bridgePid', ''),
